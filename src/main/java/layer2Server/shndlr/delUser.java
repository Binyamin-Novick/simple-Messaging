...
public class delUser extends handler{

    public delUser(Gson gson, HttpServer server,  Serverlogic sl) {
        super(gson, server, MacroDef.http.deleteUser, sl);
        this.requerdFields.add(MacroDef.http.Reqfield.token);
        this.requerdFields.add(Reqfield.username);
        this.requerdFields.add(Reqfield.password);
    }
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            long[]res =sl.deleteUser(body.get(Reqfield.token),body.get(Reqfield.username),body.get(Reqfield.password));
